...
A **workflow** is the spec of one operational loop, made real. You run a workflow on a loop: the loop is its running instantiation. Workflow procedures live in `workflows/*.md`. When a procedure needs software changes, record the software requirements, design and tasks in the selected OpenSpec change and link them from the procedure.
