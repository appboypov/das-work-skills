...
1. **Select the change**
   ...
   **Load current archive inputs before the existing archive checks:**
   ...
   ```
   Read the returned context and operation guidance. If the command fails or returns invalid JSON, report the failure and resolve the selected root or runtime prerequisite before archiving.
...
3. **Check task completion status**
   ...
   **If no tasks file exists:** Proceed without task-related warning.
   ...
   Read the change's verification evidence against its current requirements. Report failed checks or missing proof. Archive completed work only when the applicable verification succeeds. If the user explicitly archives unfinished work, identify it as unfinished historical work and preserve its incomplete tasks and findings.
