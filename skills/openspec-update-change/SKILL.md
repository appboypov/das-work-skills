...
4. **Read and reconcile**
   ...
   - Compare checked implementation tasks with the revised requirements and their evidence. Uncheck affected tasks whose completion no longer holds; update the task's expected check and record which prior evidence is invalidated in the existing planning artifacts. A fully checked list must not conceal implementation work introduced by the revision.
...
6. **Point to the next step (guidance only - NEVER act on it)**
   ...
   - Everything implemented with current evidence -> suggest `openspec-verify-change` before archive.
...
- Add a short comment to a linked Linear issue when the revision changes its relevant scope, decision or next action. Preserve its initial template description.
