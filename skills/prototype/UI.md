...
Once a variant has won, record the choice and its rationale in the owning change, then preserve the prototype as described by [prototype](SKILL.md). Under explicit implementation approval, build the selected behavior into the real page or route using production conventions and checks. The delivered page contains the selected design and its required interactions; prototype variants and their switcher remain with the referenced experiment.
