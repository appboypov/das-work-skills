...
1. Discover the authenticated Linear workspace, relevant team, existing projects, statuses, type labels and templates using [linear](../linear/SKILL.md). Establish only missing ownership choices with the user. Record stable identifiers and useful links in the project's existing configuration notes, or `docs/skills.md` when none exist. Keep credentials in the authenticated connection.
2. Read [installed setup](../workflow/references/setup.md). Establish repo-local, store-only or store-optional planning through [openspec-store](../openspec-store/SKILL.md). Install the `client-work` schema from the installed workflow skill into the resolved planning root, select it in the existing configuration, and verify native schema validation. Initialize OpenSpec only when authorized and verify `openspec context --json` resolves the intended root.
...
4. Make the installed skill location and configuration notes discoverable through the host's supported skill discovery. Preserve existing project instructions. Configure native Linear templates only when requested through [template setup](../linear/template-setup.md).
